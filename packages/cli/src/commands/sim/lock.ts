...
export default defineCommand({
  meta: {
    name: 'lock',
    description: 'Enable a facility lock (e.g. SC, FD, PN)',
  },
  args: {
    facility: {
      type: 'positional',
  ...
  run({ args }) {
    return withErrorHandling(async () => {
      await withDevice({ target: args.port, verbose: args.verbose }, async (handle) => {
        ...
        await handle.sim.setFacilityLock(args.facility, true, args.password, serviceClass)
        process.stdout.write(`${args.facility} locked\n`)
      })
    })
  },
})
